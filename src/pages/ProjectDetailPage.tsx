  ...
  const fetchData = async () => {
    try {
      const requests: Promise<any>[] = [
        ...
        isAdmin ? projectApi.get(`/api/tasks/project/${id}`) : projectApi.get("/api/tasks/my"),
      ]

      // Fetchuj usere samo ako je admin
      if (isAdmin) {
        requests.push(authApi.get('/api/auth/users'))
      }
      ...
      setTasks(isAdmin ? allTasks : allTasks.filter((t: Task) => t.projectId === Number(id)))

      // Napravi mapu userId → username
      if (isAdmin && usersRes) {
        const userMap = new Map<number, string>(
          usersRes.data.map((u: User) => [u.id, u.username])
        )
        setUsers(userMap)
      }
    } catch (err) {
            ...
            <div style={styles.taskList}>
              {tasks.filter((t) => t.status === col).map((task) => (
                <div key={task.id} style={styles.taskCard}>
                  ...
                  {isAdmin && (
                    <p style={styles.taskAssigned}>
                      👤 {task.assignedUserId ? users.get(task.assignedUserId) ?? 'Unknown' : 'Unassigned'}
                    </p>
                  )}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  taskAssigned: { fontSize: "12px", color: "#6b778c", margin: "4px 0 8px" },
